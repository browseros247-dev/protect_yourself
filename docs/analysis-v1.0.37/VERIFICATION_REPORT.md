...
| 10 (PBKDF2 100K) | `AppLockManager.kt` uses 100,000 iterations (not 10,000) | **CONFIRMED** | `app/src/main/java/protect/yourself/features/appPasswordPage/AppLockManager.kt:180` — `private const val ITERATIONS = 100_000`. Comment on lines 176-179 explicitly contrasts with NopoX's 10,000. 16-byte random salt via `SecureRandom` (line 149), `PBKDF2WithHmacSHA256` (line 160), constant-time comparison (lines 103-110). All accurate. |
